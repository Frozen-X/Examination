...
public class AdminController {
    ...
    public void export(HttpServletRequest request, HttpServletResponse response) throws Exception{
        ...
        response.setHeader("Content-Disposition", "attachment;filename=" + new String("学生信息.xlsx".getBytes("GBK"),"ISO-8859-1"));
    ...
    public String impotr(HttpServletRequest request, HttpServletResponse response) {
        //获取上传的文件
        try {
            studentService.importExcelInfo(request,response);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    @ResponseBody
    public StudentCustom editStudentUI(Integer id) throws Exception {
...
//        model.addAttribute("student", studentCustom);
    ...
    public String impotrTeacher(HttpServletRequest request, HttpServletResponse response) {
        //获取上传的文件
        try {
            teacherService.importExcelInfo(request,response);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void exportTeacher(HttpServletRequest request, HttpServletResponse response) throws Exception{
        ...
        // 指定下载的文件名，浏览器都会使用本地编码，即GBK，浏览器收到这个文件名后，用ISO-8859-1来解码，然后用GBK来显示
        // 所以我们用GBK解码，ISO-8859-1来编码，在浏览器那边会反过来执行。
        response.setHeader("Content-Disposition", "attachment;filename=" + new String("教师信息.xlsx".getBytes("GBK"),"ISO-8859-1"));
        response.setContentType("application/vnd.ms-excel;charset=UTF-8");
        response.setHeader("Pragma", "no-cache");
        response.setHeader("Cache-Control", "no-cache");
        response.setDateHeader("Expires", 0);
    ...
    @ResponseBody
    public TeacherCustom editTeacherUI(Integer id, Model model) throws Exception {
...
//        model.addAttribute("teacher", teacherCustom);
    ...
    @ResponseBody
    public CourseCustom editCourseUI(Integer id, Model model) throws Exception {
